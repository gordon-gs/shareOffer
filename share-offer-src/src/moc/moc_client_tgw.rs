...
use fproto::stream_frame::tgw_bin::{
    heartbeat::Heartbeat, logon::Logon, logout::Logout, new_order_100101::NewOrder100101,
    platform_state_info::PlatformStateInfo,
};
...
/// TCP 服务器配置
#[derive(Parser, Debug)]
#[command(name = "moc_client_tgw")]
#[command(about = "共享报盘moc_tgw客户端", long_about = None)]
struct Args {
    /// 服务器地址
    ...
    ///target_gw_id
    #[arg(short = 'T', long, default_value_t = 52)]
    target_gw_id: u16,
...
fn main() {
    ...
    if args.auto_logon {
        ...
        logon.set_default_appl_ver_id_from_string("1.11");
            ...
            Ok(line) => {
                ...
                match trimmed {
                    ...
                    "order" => {
                        ...
                        new_order.set_user_info(route_info.get_tgw_user_info());
